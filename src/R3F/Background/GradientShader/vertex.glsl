...
void main() {
  ...
  float offset = incline * mix(-.5, 0.5, uv.y);

  float noise = snoise(vec3(noiseCoords.x + u_time * .1, noiseCoords.y, u_time * 0.1));
  ...
  for (int i = 0; i < 4; i++) {
    ...
    float noise = smoothstep(noiseFloor, noiseCeil, snoise(vec3(noiseCoords.x * noiseFreq.x + u_time * noiseFlow, noiseCoords.y * noiseFreq.y, u_time * noiseSpeed + noiseSeed)));
